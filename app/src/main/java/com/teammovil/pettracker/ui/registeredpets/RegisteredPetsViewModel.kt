...
class RegisteredPetsViewModel(val petRepository: PetRepository, val rescuerRepository: RescuerRepository) : ViewModel() {
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            var rescuer = withContext(Dispatchers.IO) { rescuerRepository.getRescuer() }
            var result = withContext(Dispatchers.IO) { petRepository.getAllPatsFromRescuer(rescuer.id) }
            setView(result)
        }
    }
